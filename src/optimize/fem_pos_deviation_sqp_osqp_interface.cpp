...
#include "fem_pos_deviation_sqp_osqp_interface.hpp"
    ...
    namespace perception
    {
        namespace camera
        {
            ...
            bool FemPosDeviationSqpOsqpInterface::Solve()
            {
                ...
                {
                    AP_LERROR(FemPosDeviationSqpOsqpInterface)
                ...
                {
                    AP_LERROR(FemPosDeviationSqpOsqpInterface)
                        << "initial iteration solving fails";
                    osqp_cleanup(work);
                    c_free(data->A);
                    c_free(data->P);
                    c_free(data);
                ...
                while (pen_itr < sqp_pen_max_iter_)
                {
                    ...
                    while (sub_itr < sqp_sub_max_iter_)
                    {
                        ...
                        if (ftol < sqp_ftol_)
                        {
                            // AP_LDEBUG(FemPosDeviationSqpOsqpInterface)
                            //     << "merit function value converges at sub itr num "
                            //     << sub_itr;
                            // AP_LDEBUG(FemPosDeviationSqpOsqpInterface)
                            //     << "merit function value converges to " << cur_fvalue
                    ...
                    // AP_LDEBUG(FemPosDeviationSqpOsqpInterface)
                    //     << "ctol is " << ctol << ", at pen itr " << pen_itr;

                    if (ctol < sqp_ctol_)
                    {
                        // AP_LDEBUG(FemPosDeviationSqpOsqpInterface)
                        //     << "constraint satisfied at pen itr num " << pen_itr;
                        // AP_LDEBUG(FemPosDeviationSqpOsqpInterface)
                        //     << "constraint voilation value drops to " << ctol
                ...
                // AP_LDEBUG(FemPosDeviationSqpOsqpInterface)
                //     << "constraint not satisfied with total itr num " << pen_itr;
                // AP_LDEBUG(FemPosDeviationSqpOsqpInterface)
                //     << "constraint voilation value drops to " << ctol
                ...
                std::vector<c_float> *upper_bounds)
            {
                ...
                for (int i = 0; i < num_of_points_; ++i)
                {
                    const auto &ref_point_xy = ref_points_[i];
                    (*upper_bounds)[i * 2] =
                        ref_point_xy.first + bounds_around_refs_[i].first;
                    (*upper_bounds)[i * 2 + 1] =
                        ref_point_xy.second + bounds_around_refs_[i].second;
                    (*lower_bounds)[i * 2] =
                        ref_point_xy.first - bounds_around_refs_[i].first;
                    (*lower_bounds)[i * 2 + 1] =
                        ref_point_xy.second - bounds_around_refs_[i].second;
                ...
                const std::vector<c_float> &primal_warm_start, OSQPWorkspace **work)
            {
                ...
                {
                    AP_LERROR(FemPosDeviationSqpOsqpInterface)
                        << "failed optimization status:\t" << (*work)->info->status;
                    return false;
                }
