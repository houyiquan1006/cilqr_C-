 ...
 * Sichao Zhang <zhangsichao@senseautocom>
 */
    ...
    namespace perception
    {
        namespace camera
        {
            ...
            MpcOsqp::~MpcOsqp()
            {
                ...
                {
                    if (osqp_data_->P)
                        csc_spfree(osqp_data_->P);
                    if (osqp_data_->A)
                        csc_spfree(osqp_data_->A);
                    c_free(osqp_data_);
                }
                if (osqp_settings_)
                                                    ...
                                                    std::vector<c_int> *A_indptr)
            {
                ...
                for (size_t j = 0; j < mpc_.state_dim; ++j)
                {
                    ...
                    ++ind_A;
                    for (size_t k = 0; k < mpc_.state_dim; ++k)
                    { // row
                ...
                {
                    for (size_t i = 1; i < mpc_.horizon; ++i)
                    { // row
                        for (size_t j = 0; j < mpc_.state_dim; ++j)
                        { // col
                ...
                else if (2 == mpc_.discretize_order)
                {
                    ...
                    for (size_t i = 2; i < mpc_.horizon; ++i)
                    { // row
                        for (size_t j = 0; j < mpc_.state_dim; ++j)
                        { // col
                ...
                else if (3 == mpc_.discretize_order)
                {
                    ...
                    for (size_t i = 3; i < mpc_.horizon - 1; ++i)
                    { // row
                        for (size_t j = 0; j < mpc_.state_dim; ++j)
                        { // col
                ...
                else if (4 == mpc_.discretize_order)
                {
                    ...
                    for (size_t i = 4; i < mpc_.horizon - 2; ++i)
                    { // row
                        for (size_t j = 0; j < mpc_.state_dim; ++j)
                        { // col
                ...
                for (size_t i = 0; i < mpc_.horizon; ++i)
                {
                    ...
                    { // col
                        ...
                        for (size_t k = 0; k < mpc_.control_constraint_dim; ++k)
                        { // row
            ...
            MpcOsqp::Status MpcOsqp::CheckSolveStatus()
            {
                ...
                if (status < 0 || (status > 1))
                {
                    ...
                    {
                        AD_LERROR(MpcOsqp)
                            << "osqp error: unknown states, check StateErrorTab!";
                        return Status::ERROR_OSQP_STATUS;
                    }
                    AD_LERROR(MpcOsqp) << "osqp status error: "
                ...
                {
                    is_last_solved_ = false;
                    AD_LERROR(MpcOsqp) << "osqp solution error";
                ...
                for (size_t i = 0; i < osqp_state_dim_; ++i)
                {
                    ...
                    {
                        is_last_solved_ = false;
                        AD_LERROR(MpcOsqp) << "osqp solution has nan";
            ...
            MpcOsqp::Status MpcOsqp::Init()
            {
                ...
                if (0 != setup_error)
                {
                    ...
                    {
                        AD_LERROR(MpcOsqp) << "osqp error: check SetupErrorTab!";
                        return Status::ERROR_SETUP;
                    }
                    AD_LERROR(MpcOsqp) << "osqp setup error: "
                                           ...
                                           std::vector<double> *output)
            {
                ...
                {
                    AD_LERROR(MpcOsqp) << "osqp input error";
                ...
                if (false == is_setup_)
                {
                    ...
                    {
                        AD_LERROR(MpcOsqp) << "osqp init error";
                        return init_status;
                    }
                    is_setup_ = true;
