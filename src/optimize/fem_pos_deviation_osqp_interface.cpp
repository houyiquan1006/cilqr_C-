    ...
    namespace perception
    {
        namespace camera
        {
            ...
            bool FemPosDeviationOsqpInterface::Solve()
            {
                ...
                {
                    AP_LERROR(FemPosDeviationOsqpInterface)
                ...
                {
                    AP_LERROR(FemPosDeviationOsqpInterface) << "Failed to find solution.";
                    // Cleanup
                    osqp_cleanup(work);
                    c_free(data->A);
                    c_free(data->P);
                ...
                std::vector<c_float> *upper_bounds)
            {
                ...
                {
                    const auto &ref_point_xy = ref_points_[i];
                    upper_bounds->push_back(ref_point_xy.first +
                                            bounds_around_refs_[i].first);
                    upper_bounds->push_back(ref_point_xy.second +
                                            bounds_around_refs_[i].second);
                    lower_bounds->push_back(ref_point_xy.first -
                                            bounds_around_refs_[i].first);
                    lower_bounds->push_back(ref_point_xy.second -
                                            bounds_around_refs_[i].second);
                }
            }
